function inject (scope) {
    ...
    scope.document.body.appendChild(menuContainer);

    const fileInput = document.createElement("input");
    fileInput.type = "file";
    fileInput.accept = "video/*";
    fileInput.style.display = "none";
    scope.document.body.appendChild(fileInput);
    ...
    let _enumerateDevices = scope.navigator.mediaDevices.enumerateDevices;
    ...
    let _getUserMedia = scope.navigator.mediaDevices.getUserMedia;
    scope.navigator.mediaDevices.getUserMedia = async function (constraints) {
        if (!constraints) {
            return _getUserMedia.call(scope.navigator.mediaDevices, constraints);
        }
    ...
    scope.document.addEventListener("mousemove", function (event) {
        if (isDragging) {
            toggleButton.style.top = `${event.clientY - offsetY}px`;
        }
    });

    scope.document.addEventListener("mouseup", function () {
        isDragging = false;
    });

    scope.document.body.appendChild(toggleButton);
...
function zoomCheck() {
    if (location.host == "app.zoom.us" && !top?.injected) {
        let meetingFrame = document.getElementById("webclient");
        if (meetingFrame?.contentWindow) {
            inject(meetingFrame?.contentWindow);
            top.injected = true;
        } else {
            inject(window);
            top.injected = true;
        }
    } else if (location.host !== "app.zoom.us") {
        inject(window);
    }
}
